...
const LikedBy = (props) => {
  let bookLikedByPeep = props.chart.filter(item => item.bookLikedID === props.ID);
  console.log(bookLikedByPeep)
  return(
    <div>
      <h2>{props.books[props.ID].name}</h2>
      <p>Liked By:</p>
      <ul>
        {bookLikedByPeep.length > 0 ? 
          bookLikedByPeep.map(item => <li>{props.peeps[item.peepsID].name}</li>) :
          <p>No One Liked it</p>}
      </ul>
    </div>
  )
  ...
  return(
    <div>
      {Object.values(books).map(item => {
        <LikedBy ID={item.id} chart={chart} peeps={peeps} books={books} />
      })}
    </div>
  )
}
